...
export class MainPageComponent implements OnInit {
  ...
  constructor(private userService: UserServiceService, private requestService: RequestsServiceService,
    private mainRouter: Router, private domSanitizer: DomSanitizer, private bookService: BookServiceService) { }
  ...
  ngOnInit(): void {
    ...
    this.bookService.getAll().subscribe((data: Book[]) => {
      this.allBooks = data;
      this.sortByLoanDate();
      this.images = [this.allBooks[0].picture, this.allBooks[1].picture, this.allBooks[2].picture]
    })
  ...
  sortByLoanDate() {
    this.allBooks = this.allBooks.sort((book1, book2) => {
      ...
      subscribe((logginUser: User) => {
        ...
        this.userService.checkEmail(this.registerEmail).
          subscribe((logginUser: User) => {
            ...
            this.requestService.insertRequest(this.registerUsername, this.registerPassword, this.registerName, this.registerLastName, this.registerAddress, this.registerContact, this.registerEmail, 'citalac', this.picture, 'na cekanju').subscribe(respObj => {
