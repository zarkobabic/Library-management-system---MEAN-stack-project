...
registerNewBook(){
  ...
  if(this.newBookArrayGenre.length == 0 || this.newBookArrayGenre == null ){
    this.newBookArrayGenreError = "Morate uneti bar jedan zanr";
    return false;
  }
...
//dohvati id koji treba

this.bookRequestsService.getAll().subscribe((data: BookRequest[])=>{
  this.bookRequestsArray = data;


    this.bookRequestsArray = this.bookRequestsArray.sort((bookRequest1,bookRequest2)=>{
      if(bookRequest1.idBookRequest > bookRequest2.idBookRequest){
        return -1;
      }
      else{
        if(bookRequest1.idBookRequest == bookRequest2.idBookRequest){
    ...
    let newId;
    if(this.bookRequestsArray == null || this.bookRequestsArray.length == 0){
     newId = 1;
    }else{
     newId = this.bookRequestsArray[0].idBookRequest + 1;
    }

  //Za ignorisanje space pri parsiranju stringa
    let arrayOfWriters: string[] = this.newBookWriter.split(",").map((item: string) => item.trim())

    this.bookRequestsService.insertRequest(newId,this.readerUser.username, this.newBookTitle, arrayOfWriters, this.newBookArrayGenre, this.newBookPublisher, this.newBookPubYear, this.newBookLanguage, this.pictureBook).subscribe(respObj=>{
      if(respObj['message'] == "ok"){
